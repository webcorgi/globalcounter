 ...
 */
export default async function getUserCityFromGeolite(ip:string):Promise<GeoLocationInfo> {
    ...
    // 국가 ISO 코드를 추출합니다.
    const isoCode = response.country?.isoCode ?? 'Unknown';

    // 국가 이름을 추출합니다.
    const countryName = response.country?.names.en ?? 'Unknown';

    // 방문자의 국가 ISO 코드와 국가 이름을 포함한 객체를 반환합니다.
    return { isoCode:isoCode, countryName:countryName };
}